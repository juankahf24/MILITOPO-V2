...
(function(){
  ...
  function updateControlUi(status="status",detail={}){
    const api=controlsApi(),snap=api?.snapshot?.()||{},card=el("m2raceControlCard"),nextEl=el("m2raceNextControl"),progressEl=el("m2raceControlProgress"),distanceEl=el("m2raceControlDistance"),statusEl=el("m2raceControlStatus"),qrBtn=el("m2raceQrOpen");
    if(!card)return;
    ...
    const completed=Math.max(0,Number(detail.completedCount??snap.completedCount??0)),expected=Math.max(0,Number(detail.expectedCount??snap.expectedCount??0)),next=detail.nextControl??snap.nextControl??null;
    progressEl.textContent=`${completed} / ${expected}`;
    if(!next){nextEl.textContent="LLEGADA";nextEl.classList.add("complete");distanceEl.textContent="—";qrBtn.hidden=true;statusEl.className="m2race-control-status ok";statusEl.textContent="✅ Todas las balizas están validadas. Dirígete a LLEGADA y finaliza el recorrido.";return;}
    nextEl.classList.remove("complete");nextEl.textContent=String(next.checkpointId||"—");qrBtn.hidden=false;
    if(detail.distanceM!=null)distanceEl.textContent=fmtMeters(detail.distanceM);
    if(status==="passed"||status==="qr_passed"){const pass=detail.pass||{};statusEl.className="m2race-control-status ok";statusEl.textContent=`✅ ${pass.checkpointId||"Baliza"} validada por ${String(pass.source||"").toUpperCase()==="QR"?"QR":"GPS"}. Siguiente: ${detail.nextAfter?.checkpointId||snap.nextControl?.checkpointId||"LLEGADA"}.`;}
    else if(status==="offline"){statusEl.className="m2race-control-status warn";statusEl.textContent=detail.message||"Baliza guardada en el dispositivo. Se sincronizará al volver la cobertura.";}
    else if(status==="sync_error"||status==="qr_error"){statusEl.className="m2race-control-status warn";statusEl.textContent=detail.message||"No se pudo sincronizar la baliza. El registro local se conserva.";}
    else if(status==="syncing"){statusEl.className="m2race-control-status";statusEl.textContent="Sincronizando paso por baliza…";}
    else if(status==="gps"){
      statusEl.className="m2race-control-status";
      if(detail.distanceM==null){statusEl.textContent="Esperando coordenadas de la siguiente baliza. Puedes usar el QR en cualquier ubicación si necesitas respaldo.";}
      else if(detail.accuracyOk===false){statusEl.className="m2race-control-status warn";statusEl.textContent=`Precisión GPS insuficiente: ±${Math.round(Number(detail.accuracyM||0))} m. Para validar por GPS necesitas ±10 m o mejor y estar a 10 m o menos. El QR sigue disponible.`;}
      else if(Number(detail.distanceM)>10){statusEl.textContent=`Siguiente ${next.checkpointId} · GPS ±${Math.round(Number(detail.accuracyM||0))} m · estás a ${Math.round(Number(detail.distanceM))} m. Acércate hasta 10 m o menos, o usa el QR.`;}
      else{statusEl.textContent=`Siguiente ${next.checkpointId} · dentro de 10 m y con precisión válida. Validando por GPS…`;}
    }
  ...
  window.addEventListener("militopo:v2-control-status",e=>{const d=e.detail||{};if(d.status==="qr_closed"){const panel=el("m2raceQrPanel");if(panel)panel.hidden=true;return;}updateControlUi(d.status||"status",d);});
