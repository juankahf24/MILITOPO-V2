/* MILITOPO V2 · G4 · Mapa Live del organizador.
   Visualiza balizas, posiciones GPS en tiempo real y la traza del corredor seleccionado.
   Solo lectura. No modifica la carrera ni el track. */
import "../bootstrap.js";
...
const VERSION = "v2-g4-organizer-live-map-20260924";
...
function ensureStyles() {
  if (document.getElementById("m2G4MapStyles")) return;
  const style = document.createElement("style");
  style.id = "m2G4MapStyles";
  style.textContent = `
    .m2-g4map{margin:16px 0;padding:16px;border-radius:20px;border:1px solid rgba(126,220,150,.30);background:linear-gradient(180deg,rgba(8,26,14,.94),rgba(5,17,9,.96))}
    .m2-g4map-head{display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap}
    .m2-g4map-title{font-weight:900;letter-spacing:.08em;color:#e6f6d7}
    .m2-g4map-chip{padding:6px 11px;border-radius:999px;border:1px solid rgba(126,220,150,.36);font-size:.72rem;font-weight:900}
    .m2-g4map-message{margin:10px 0 12px;font-size:.78rem;line-height:1.45;opacity:.84}
    .m2-g4map-toolbar{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:8px;margin-bottom:10px}
    .m2-g4map-select,.m2-g4map-btn{min-height:38px;border-radius:11px;border:1px solid rgba(255,255,255,.15);background:rgba(0,0,0,.22);color:#f7f2e8;padding:0 11px;font:inherit;font-size:.70rem;font-weight:800}
    .m2-g4map-btn{cursor:pointer;white-space:nowrap}.m2-g4map-btn:disabled{opacity:.45;cursor:not-allowed}
    .m2-g4map-canvas{height:410px;border-radius:16px;overflow:hidden;border:1px solid rgba(255,255,255,.12);background:#182017;position:relative}
    .m2-g4map-empty{position:absolute;inset:0;display:grid;place-items:center;padding:24px;text-align:center;font-size:.76rem;opacity:.68;pointer-events:none;z-index:400}
    .m2-g4map-legend{display:flex;gap:12px;flex-wrap:wrap;margin-top:9px;font-size:.62rem;opacity:.75}
    .m2-g4map-legend span{display:inline-flex;align-items:center;gap:5px}
    .m2-g4-runner-dot{width:20px;height:20px;border-radius:50%;display:grid;place-items:center;border:2px solid #f7f2e8;background:#5a9ad6;box-shadow:0 2px 8px rgba(0,0,0,.45);font-size:9px;font-weight:900;color:white}
    .m2-g4-runner-dot.stale{background:#6c746b}.m2-g4-runner-dot.finished{background:#6fa45f}
    .m2-g4-checkpoint{min-width:22px;height:22px;padding:0 4px;border-radius:999px;display:grid;place-items:center;border:2px solid rgba(255,255,255,.9);background:#9b6b2f;color:white;font-size:9px;font-weight:900;box-shadow:0 2px 7px rgba(0,0,0,.38)}
    .m2-g4-checkpoint.start{background:#397d55}.m2-g4-checkpoint.finish{background:#934d4d}
    .m2-g4map .leaflet-control-attribution{font-size:9px}
    @media(max-width:700px){.m2-g4map-toolbar{grid-template-columns:1fr 1fr}.m2-g4map-select{grid-column:1/-1}.m2-g4map-canvas{height:340px}}
  `;
...
function ensurePanel() {
  ...
  panel.className = "m2-g4map";
  panel.innerHTML = `
    <div class="m2-g4map-head">
      <div class="m2-g4map-title">🗺️ MAPA LIVE V2 · ORGANIZADOR</div>
      <div id="m2G4MapChip" class="m2-g4map-chip">ESPERANDO</div>
    </div>
    <div id="m2G4MapMessage" class="m2-g4map-message">Carga un evento para preparar el mapa Live.</div>
    <div class="m2-g4map-toolbar">
      <select id="m2G4RunnerSelect" class="m2-g4map-select" aria-label="Corredor para mostrar su traza"><option value="">TRAZA · NINGÚN CORREDOR</option></select>
      <button id="m2G4FitBtn" class="m2-g4map-btn" type="button">ENCUADRAR TODO</button>
      <button id="m2G4TrackBtn" class="m2-g4map-btn" type="button" disabled>VER TRAZA</button>
    </div>
    <div id="m2G4Map" class="m2-g4map-canvas"><div id="m2G4MapEmpty" class="m2-g4map-empty">Esperando puntos o posiciones GPS…</div></div>
    <div class="m2-g4map-legend"><span>● Corredor con GPS</span><span>○ Señal antigua</span><span>◆ Baliza / salida / llegada</span></div>`;
  ...
  panel.querySelector("#m2G4RunnerSelect")?.addEventListener("change", event => {
    state.selectedUid = String(event.target.value || "");
    bindTrack().catch(() => {});
    renderToolbar();
  });
  panel.querySelector("#m2G4TrackBtn")?.addEventListener("click", () => {
    const select = panel.querySelector("#m2G4RunnerSelect");
  ...
  panel.querySelector("#m2G4FitBtn")?.addEventListener("click", () => fitAll(true));
  state.panel = panel;
  render();
...
function ensureMap() {
  ...
  const node = state.panel.querySelector("#m2G4Map");
  if (!L || !node) {
  ...
  state.map = L.map(node, { zoomControl: true, preferCanvas: true }).setView([40.2, -3.7], 5);
...
function redrawCheckpoints() {
  ...
  Object.values(state.checkpoints || {}).forEach(cp => {
    ...
    const icon = L.divIcon({
      className: "",
      html: `<div class="m2-g4-checkpoint ${cls}">${esc(label)}</div>`,
...
function redrawRunners() {
  ...
  Object.entries(state.participants || {}).forEach(([uid, row]) => {
    ...
    const icon = L.divIcon({
      className: "",
      html: `<div class="m2-g4-runner-dot ${finished ? "finished" : stale ? "stale" : ""}">${esc(initial)}</div>`,
...
function updateEmpty() {
  const empty = state.panel?.querySelector("#m2G4MapEmpty");
  if (!empty) return;
...
function renderToolbar() {
  ...
  if (!panel) return;
  const select = panel.querySelector("#m2G4RunnerSelect");
  const trackBtn = panel.querySelector("#m2G4TrackBtn");
...
function render() {
  ...
  if (!panel) return;
  const chip = panel.querySelector("#m2G4MapChip");
  const message = panel.querySelector("#m2G4MapMessage");
  if (!canManage()) {
...
async function bindCheckpoints() {
  ...
  state.unsubCheckpoints = onSnapshot(collection(firestore, "events", state.eventId, "checkpoints"), snap => {
    ...
    console.warn("[MILITOPO G4] checkpoints", error);
...
async function bindParticipants() {
  ...
  state.unsubParticipants = onValue(ref(database, path), snap => {
    ...
    console.error("[MILITOPO G4] participants", error);
...
async function bindTrack() {
  ...
  state.unsubTrack = onValue(ref(database, path), snap => {
    ...
    console.error("[MILITOPO G4] track", error);
...
async function bindEvent(eventId) {
  ...
  try {
    ...
    state.unsubActive = onValue(ref(database, activePath), snap => {
      ...
      console.error("[MILITOPO G4] activeRun", error);
      state.lastError = "No se pudo conectar el mapa con la sesión Live V2.";
      render();
    });
  } catch (error) {
    console.error("[MILITOPO G4] bindEvent", error);
