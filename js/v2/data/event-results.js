...
function ensurePanel() {
  ...
  panel.querySelector("#m2H2Refresh")?.addEventListener("click", () => bindResults(true));
  state.panel = panel;
  render();
  return panel;
}
...
function init() {
  ...
  addEventListener("militopo:v2-auth-signed-out", () => {
    state.auth = null; state.rows = []; state.eventId = ""; state.eventStatus = ""; stopResults(); render();
  });
  ...
  addEventListener("militopo:v2-event-status-changed", event => {
    ...
    setTimeout(() => bindResults(true), 350);
  });
  addEventListener("online", () => bindResults(true));
