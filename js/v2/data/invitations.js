...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .m2-invites-create{background:linear-gradient(180deg,#f6d285,#d99c38)!important;color:#1b160c!important}.m2-invites-status{margin:9px 0;font-size:.82rem;line-height:1.4;opacity:.86}
    ...
    @media(max-width:680px){.m2-invites-form{grid-template-columns:1fr}.m2-invites-form button,.m2-invites-bulk button{width:100%}.m2-invite-item{grid-template-columns:1fr}.m2-invite-actions{justify-content:stretch;max-width:none}.m2-invite-actions button{flex:1 1 45%}}
  `;
...
function ensurePanel() {
  ...
  panel.innerHTML = `
    ...
    <div class="m2-invites-bulk">
      <label>Invitar a varios
        <textarea id="m2InviteBulk" autocapitalize="none" spellcheck="false" placeholder="@juan\n@maria\npedro@correo.com"></textarea>
      </label>
      <small>Pega usuarios o correos separados por líneas, espacios, comas o punto y coma. MILITOPO elimina duplicados y omite los que ya tengan invitación pendiente.</small>
  ...
  state.create.addEventListener("click", () => createTargets([state.target.value], false));
  state.bulkCreate.addEventListener("click", () => createTargets(parseBulk(state.bulk.value), true));
...
function paint(message = "") {
  ...
  if (!canManage()) {
    ...
    state.create.disabled = true; state.bulkCreate.disabled = true; state.target.disabled = true; state.bulk.disabled = true; return;
  }
  state.target.disabled = state.busy; state.bulk.disabled = state.busy;
  if (!state.event) {
    chip.textContent = currentEventId() ? "COMPROBANDO" : "SIN EVENTO";
    state.status.textContent = message || "Carga un evento PREPARADO o PUBLICADO."; state.create.disabled = true; state.bulkCreate.disabled = true; return;
  }
...
async function createTargets(rawTargets, bulkMode) {
  ...
  try {
    ...
    if (!bulkMode && created) state.target.value = "";
    if (bulkMode && created) state.bulk.value = "";
    await loadInvitations();
    const pieces = [`✅ ${created} invitación${created === 1 ? "" : "es"} creada${created === 1 ? "" : "s"}`];
    if (alreadyJoined) pieces.push(`${alreadyJoined} ya estaba${alreadyJoined === 1 ? "" : "n"} unido${alreadyJoined === 1 ? "" : "s"} al evento`);
    if (skipped) pieces.push(`${skipped} ya tenía${skipped === 1 ? "" : "n"} invitación pendiente`);
    if (errors.length) pieces.push(`${errors.length} no válida${errors.length === 1 ? "" : "s"}: ${errors.slice(0,3).join(" · ")}${errors.length > 3 ? "…" : ""}`);
    paint(pieces.join(" · "));
...
function init() {
  ensurePanel();
  globalThis.addEventListener("militopo:v2-auth-ready", event => { state.auth = event?.detail || globalThis.MILITOPO_V2_AUTH || null; state.loadedEventId = ""; scheduleLoad(150, true); });
