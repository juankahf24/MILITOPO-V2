...
self.addEventListener("activate",event=>event.waitUntil((async()=>{if(self.registration.navigationPreload)try{await self.registration.navigationPreload.enable()}catch(_){}const keys=await caches.keys();await Promise.all(keys.filter(k=>(k.startsWith("militopo-v2-orientacion-")||k.startsWith("militopo-v2-page-snapshot-"))&&k!==MILITOPO_CACHE&&k!==RUNTIME_CACHE&&k!=="militopo-v2-page-snapshot-v2-d3-organizer-20260921").map(k=>caches.delete(k)));await self.clients.claim()})()));
...
self.addEventListener("message",event=>{
  if(event.data&&event.data.type==="MILITOPO_CLEAR_OLD_CACHES"){
    event.waitUntil((async()=>{const keys=await caches.keys();await Promise.all(keys.filter(k=>(k.startsWith("militopo-v2-orientacion-")||k.startsWith("militopo-v2-page-snapshot-"))&&![MILITOPO_CACHE,RUNTIME_CACHE,"militopo-v2-page-snapshot-v2-d3-organizer-20260921"].includes(k)).map(k=>caches.delete(k)))})());
  }
});
